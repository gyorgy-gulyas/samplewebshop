...
namespace CustomerManagement.Customers.Customer
{
	...
	public partial class PostalAddress : IEquatable<PostalAddress>, IValidable
	{
		...
		public virtual bool Validate( IList<IValidationError> errors )
		{
			...
			if (country.Length != 2)
				errors.Add( new ValidationError { TypeOfEntity = "PostalAddress", MemberOfEntity = "country", ErrorText = "country must satisfy: len(value) == 2" } );

			if (postalCode.Length <= 0)
				errors.Add( new ValidationError { TypeOfEntity = "PostalAddress", MemberOfEntity = "postalCode", ErrorText = "postalCode must satisfy: len(value) > 0" } );

			if (city.Length <= 0)
				errors.Add( new ValidationError { TypeOfEntity = "PostalAddress", MemberOfEntity = "city", ErrorText = "city must satisfy: len(value) > 0" } );

			if (street.Length <= 0)
				errors.Add( new ValidationError { TypeOfEntity = "PostalAddress", MemberOfEntity = "street", ErrorText = "street must satisfy: len(value) > 0" } );
