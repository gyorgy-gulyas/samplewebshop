...
namespace Sales.OrderManagement
{
	...
	[SwaggerTag( "The second version lives next to the first on purpose: this is how a published interface is carried forward without breaking the callers still on v1." )]
			...
			using(LogContext.PushProperty( "Scope", "OrderIF_v2.getOrder" ))
			{
				...
				try
				{
					...
					if( response.IsSuccess() == true )
					{
						...
						{
							return StatusCode(Statuses.NotImplemented.ToHttp(), new List<ServiceKit.Net.Error>() { new() { MessageText = "Not handled reponse in REST Controller when calling 'OrderIF_v2.getOrder'" } } );
						}
					}
					else
					{
			...
			using(LogContext.PushProperty( "Scope", "OrderIF_v2.placeOrder" ))
			{
				...
				try
				{
					...
					if( response.IsSuccess() == true )
					{
						...
						{
							return StatusCode(Statuses.NotImplemented.ToHttp(), new List<ServiceKit.Net.Error>() { new() { MessageText = "Not handled reponse in REST Controller when calling 'OrderIF_v2.placeOrder'" } } );
						}
					}
					else
					{
